...
export type Action = ExtractReturn<typeof pauseStatus>;
...
export const purchasePrep = () => async (
  dispatch: Function,
  getState: GetState
) => {
  ...
  if (!(await st.verifyTransfer(0, account, 1))) {
    dispatch(
      ui.confirm(
        <div>
          Your wallet address cannot participate into this token sale. Please
          contact the Issuer of the {token.ticker} security token for
          resolution.
        </div>,
        () => {},
...
export const purchase = () => async (
  ...
  if (details.isPolyFundraise) {
    if (!isSufficientAllowance) {
      polyConfirmText = (
        <p>
          Completion of your {token.ticker} token purchase will require two
          wallet transactions:
          <br />• The first transaction will be used to approve the payment of
          POLY
          <br />• The second transaction will be used to transfer the POLY and
          complete the purchase of {token.ticker} tokens.
        </p>
      );
    } else {
      polyConfirmText = (
        <p>
          Usually completion of your {token.ticker} token purchase require two
          wallet transactions:
          <br />• The first transaction to approve the payment of POLY
          <br />• The second transaction to transfer the POLY and complete the
          purchase of {token.ticker} tokens.
          <br />
          But you already approved POLY payment earlier, so now you will have to
          sign only second transaction.
        </p>
      );
    }
  }
  ...
  dispatch(
    ui.confirm(
      <div>
        ...
        <strong>
          {ui.thousandsDelimiter(tokens)} {token.ticker}
        </strong>{' '}
        tokens for{' '}
        <strong>
          {ui.thousandsDelimiter(value)}{' '}
          {details.isPolyFundraise ? 'POLY' : 'ETH'}
        </strong>
        .{polyConfirmText}
      </div>,
      async () => {
        if (!(await st.verifyTransfer(0, account, tokens))) {
          dispatch(
            ui.confirm(
              <div>
                You are not allowed to receive {ui.thousandsDelimiter(tokens)}{' '}
                {token.ticker}.<br />
                Try lower value.
              </div>,
              () => {},
        ...
        if (!details.tokensSold.add(tokens).lte(details.cap)) {
          dispatch(
            ui.confirm(
              <div>
                The maximum number of tokens that you can now purchase is{' '}
                <strong>
                  {ui.thousandsDelimiter(details.cap.minus(details.tokensSold))}
                </strong>
                .
              </div>,
              () => {},
        ...
        dispatch(
          ui.tx(
            [
              ...(details.isPolyFundraise && !isSufficientAllowance
            ...
            `Congratulations! You Completed Your Purchase of ${ui.thousandsDelimiter(
              tokens
            )} of ${token.ticker} Tokens`,
            () => {
