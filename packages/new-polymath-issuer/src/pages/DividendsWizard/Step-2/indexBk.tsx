...
const Step2Presenter = ({
  ...
  const deleteRow = (investorAddress: string) => {
    setWithholdingList(
      _.remove(withholdingList, item => {
        return item.data.investorWalletAddress.value !== investorAddress;
      })
    );
  };
  ...
  const handleCsvModalConfirm = useCallback(
    () => {
      ...
      const addedEntries =
        values.taxWithholdingsCsv &&
        values.taxWithholdingsCsv.map((csvRow: csvParser.ResultRow) => ({
